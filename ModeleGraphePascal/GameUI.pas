...
begin
  DrawText(PChar(Key + ':'), X, Y, 18, LIGHTGRAY);  // Gris clair pour les clés
  DrawText(PChar(Value), X + 110, Y, 18, WHITE);    // Blanc pour les valeurs
end;
...
begin
  ...
  // Dessiner le fond gris foncé
  DrawRectangleRec(PanelRect, DARKGRAY);

  // Dessiner la bordure
  DrawRectangleLinesEx(PanelRect, 2, LIGHTGRAY);

  // Dessiner le titre
  DrawText(PChar('INFORMATIONS'), Trunc(PanelRect.x) + 10, 10, 20, WHITE);
  ...
  begin
    DrawText(PChar('Cliquez sur un element'), X, Y, 18, LIGHTGRAY);
    DrawText(PChar('pour voir ses details.'), X, Y + 25, 18, LIGHTGRAY);
    Exit;
  end;
...
begin
  ...
  // Dessiner le titre
  DrawText(PChar('LOGS SYSTEME'), 10, Trunc(PanelRect.y) + 10, 20, WHITE);

  // Dessiner le message de log en vert pour le contraste
